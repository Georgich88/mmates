...
import com.mmates.sherdogparser.models.Promotion;

/**
 * Created by gz on 20-Aug-16. Parser to parse an promotion
 */
public class PromotionParser implements SherdogParser<Promotion> {
	...
	public List<Promotion> downloadPromotions(int start, int depth)
			throws IOException, ParseException, SherdogParserException {
		...
		if (trs.size() > 0) {
			...
			trs.forEach(tr -> {
				...
				try {
					...
					logger.error(err.getMessage());
					err.printStackTrace();
				} catch (SherdogParserException err) {
					logger.error(err.getMessage());
					err.printStackTrace();
				}
